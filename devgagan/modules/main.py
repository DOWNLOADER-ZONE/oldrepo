...
from config import API_ID, API_HASH, FREEMIUM_LIMIT, PREMIUM_LIMIT, OWNER_ID, DEFAULT_SESSION , DIAMOND_LIMIT, GOLD_LIMIT , SILVER_LIMIT , FREE_LINK_LIMIT
...
from devgagan.core.func import subscribe, chk_user , get_link , userbot_join
...
async def single_link(_, message):
    ...
    if free_check == 1 : 
        ...
        if link_count == FREE_LINK_LIMIT:
            await message.reply(f"You have Reached the limit of {FREE_LINK_LIMIT} per day for Free users Refresh at 12 am Ist"    )
            return
        else:
    ...
    if user_banned:
        return await message.reply(f"Sorry Your Are banned by my Owner ",reply_markup=buttons)
    ...
    #     await message.reply(f"Upgrade to premium for access.\nUser ID: {user_id}")
    #     return
...
async def batch_link(_, message):
    ...
    elif user_plan == "diamond":
        max_batch_size = DIAMOND_LIMIT
    elif user_plan == "gold":
        max_batch_size = GOLD_LIMIT
    elif user_plan == "silver":
        max_batch_size = SILVER_LIMIT
    elif user_id == str(OWNER_ID):
        max_batch_size = DIAMOND_LIMIT    
    else:
        max_batch_size = GOLD_LIMIT  # Default for gold users
    ...
    join_button = InlineKeyboardButton("Join Channel", url="https://t.me/SungJinBotz")
    keyboard = InlineKeyboardMarkup([[join_button]])
    pin_msg = await app.send_message(
        user_id,
        f"Batch process started ⚡\nUser ID: {user_id}\nProcessing: 0/{cl}\n\n**Powered by Team SUNG**",
